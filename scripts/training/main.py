...
def training(data, testing_subject, config_name):
    ...
    print("Total rotations: " + str(rotations))
    ...
    for rot in range(rotations):
        ...
        print("------------------------------------")
        print("\033[91mStatus of Rotation\033[0m: " + str(rot))
        print("Test with:", end =" ")
        print(*folds[rot]['test'], sep=', ')
        print("Validation with:", end =" ")
        print(*folds[rot]['val'], sep=', ')
        print("Train with:", end =" ")
        print(*folds[rot]['train'], sep=', ')
        ...
        print("fold %s Time lapse:" % (rot), time_lapse)

        if not os.path.exists("%s/%s_%s_test_%s_val_%s" % (results_path, model_type, rot, test_subject, val_subject)):
            os.makedirs("%s/%s_%s_test_%s_val_%s" % (results_path, model_type, rot, test_subject, val_subject))
        if not os.path.exists("%s/%s_%s_test_%s_val_%s/model" % (results_path, model_type, rot, test_subject, val_subject)):
            os.makedirs("%s/%s_%s_test_%s_val_%s/model" % (results_path, model_type, rot, test_subject, val_subject))
        if not os.path.exists("%s/%s_%s_test_%s_val_%s/true_label" % (results_path, model_type, rot, test_subject, val_subject)):
            os.makedirs("%s/%s_%s_test_%s_val_%s/true_label" % (results_path, model_type, rot, test_subject, val_subject))
        if not os.path.exists("%s/%s_%s_test_%s_val_%s/file_name" % (results_path, model_type, rot, test_subject, val_subject)):
            os.makedirs("%s/%s_%s_test_%s_val_%s/file_name" % (results_path, model_type, rot, test_subject, val_subject))
        if not os.path.exists("%s/%s_%s_test_%s_val_%s/prediction" % (results_path, model_type, rot, test_subject, val_subject)):
            os.makedirs("%s/%s_%s_test_%s_val_%s/prediction" % (results_path, model_type, rot, test_subject, val_subject))





        print("Saving model for fold " + str(rot))
        model_ready.save("%s/%s_%s_test_%s_val_%s/model/%s_%s_test_%s_val_%s_resnet50.h5" % (results_path, model_type, rot, test_subject, val_subject, model_type, rot, test_subject, val_subject))
        ...
        print("Saving evaluation of test for fold " + str(rot))
        with open('%s/%s_%s_test_%s_val_%s/%s_%s_test_%s_val_%s_evaluation_test.csv' % (results_path, model_type, rot, test_subject, val_subject, model_type, rot, test_subject, val_subject), 'w') as f:
            writer = csv.writer(f)
