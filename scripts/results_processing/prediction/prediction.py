    ...
    for model in models:
        ...
        for subject in image_paths:
            ...
            for label in image_paths[subject]:
                ...
                timing_results[model][subject][label] = time.time() - srt
        ...
        for subject in prediction_results[model]:
            for label in prediction_results[model][subject]:
                ...
                dirpath = os.path.join(
                    out_vals,
                    f"Test_subject_{subject}/config_{model_i}_{model}/{model}_{subject_i}_test_{subject}_val_{label}/"
                )
                ...
                prefix = f"{model}_{model_i}_test_{subject}_val_{label}_val"
                
                # Print the prediction probability values
                preds = pd.DataFrame(prediction_results[model][subject][label])
                ...
                trues = pd.DataFrame([label]*len(input_filepaths[subject][label]))
                ...
                trues = pd.DataFrame([class_names.index(label)]*len(input_filepaths[subject][label]))
                ...
                # Print the true label index values
                files = pd.DataFrame(input_filepaths[subject][label])
                ...
                # Print the true label index values
                time = pd.DataFrame([timing_results[model][subject][label]])
                ...
                for i in range(len(input_filepaths[subject][label])):
                    ...
                    predicted_labels = pd.concat([predicted_labels, pd.DataFrame.from_dict({
                        ...
                        'filename': [img_file],
                        'true_label': [label],
                        'true_label_index': [class_names.index(label)],
                        'pred_label': [max_label],
                        'pred_label_index': [max_index],
                        'match': [class_names.index(label) == max_index]                        
                    })], ignore_index=True)
